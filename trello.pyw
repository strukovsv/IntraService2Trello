...
#param["ConnectString"]   = "DRIVER={SQL Server};SERVER=172.16.10.252;DATABASE=IntraService;UID=ETK\SergeySt;PWD=;Trusted_Connection=yes"
param["ConnectString"]   = "DRIVER={SQL Server Native Client 11.0};SERVER=172.16.10.252;DATABASE=IntraService;UID=ETK\SergeySt;PWD=;Trusted_Connection=yes"
...
class DB_reader():
    ...
    def get_task(self, task_id, userid):
        ...
        for task in self.select("""
            ...
            if (status == "Проверка заявителем"):
                status = "Тестирование"
            if (status == "Исполнитель назначен"):
                status = "Открыта"
            ...
            if (object["service"] == "Работает неправильно/зависает"):
                object["service"] = "Инциденты"     
                
            if (object["service"] == "Проблемы с онлайн остатком"):
                object["service"] = "Инциденты"     
    ...
    try:    
        ...
        for user in db.select("""
                              SELECT UserId,
                                    ...
                                    ViewClosed 
                              FROM UserTrello ut WHERE userid <> 0/*where userid = 3605*/
                              ORDER BY 1
                              """):
            try:
